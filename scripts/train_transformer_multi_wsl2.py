...
def create_dataloaders(train_df, val_df, num_workers=8, use_temporal_pairs=False):
    """
    ...
    Parameters
    ----------
    train_df, val_df : pl.DataFrame
        Training and validation data splits
    num_workers : int
        Number of data loading workers
    use_temporal_pairs : bool, default=False
        If True, use TimeAwareTrajectoryDataset (temporal pair contrastive learning).
        If False (default), use AugmentationContrastiveDataset (augmentation-only,
        more windows, no wasted features_t1 computation).
    """
    ...
    if use_temporal_pairs:
        # Legacy: temporal pair contrastive learning (requires features_t and features_t1)
        print("   🔗 Using TimeAwareTrajectoryDataset (temporal pairs)")
        ...
        # Default: augmentation-based contrastive learning (single windows, more data)
        print("   🎯 Using AugmentationContrastiveDataset (augmentation-only, all valid windows)")
        train_dataset = AugmentationContrastiveDataset(
            train_df, window_size=window_size, overlap=overlap, min_track_length=min_track_length
        )
        val_dataset = AugmentationContrastiveDataset(
            val_df, window_size=window_size, overlap=overlap, min_track_length=min_track_length
        )
        dataset_label = "windows"
    ...
    print(f"   ✅ Train: {len(train_dataset):,} {dataset_label} → {len(train_loader):,} batches")
    print(f"   ✅ Val:   {len(val_dataset):,} {dataset_label} → {len(val_loader):,} batches")
...
def train_model(model_index, config, train_loader, val_loader, paths, args):
    ...
    # Determine max epochs (use override if provided, otherwise from config)
    max_epochs = args.epochs if args.epochs is not None else TRANSFORMER_TRAINING["epochs"]
    ...
    if args.mode in ["auto", "resume", "continue"]:
        ...
        if checkpoint_to_load:
            if resume_epoch < max_epochs:
                ...
                print(f"   Will continue training to {max_epochs} epochs ({max_epochs - resume_epoch} more)")
            else:
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use legacy TimeAwareTrajectoryDataset (temporal pair contrastive). "
             "Default: False (uses AugmentationContrastiveDataset — more windows, augmentation-only)",
    )
    
    args = parser.parse_args()
    
    # Set random seeds for reproducibility (MATCHES NOTEBOOK: RANDOM_SEED=42)
    RANDOM_SEED = 42
    ...
    # Ensure deterministic behavior (may slow down slightly)
    ...
    train_loader, val_loader = create_dataloaders(
        train_df, val_df, num_workers=num_workers, use_temporal_pairs=args.temporal_pairs
    )
    ...
    if results:
        ...
        for r in results:
            val_loss_str = f"{r['final_val_loss']:.4f}" if r.get('final_val_loss') is not None else 'N/A'
            print(f"   {r['model_name']}: val_loss={val_loss_str}")
