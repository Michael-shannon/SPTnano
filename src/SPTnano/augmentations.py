...
class TrajectoryAugmentations:
    ...
    def time_warping(self, x: torch.Tensor) -> torch.Tensor:
        ...
        warp_steps = torch.cumsum(
            torch.exp(torch.randn(T, device=x.device) * self.time_warp_sigma), 
            dim=0
        )
    ...
    def magnitude_warping(self, x: torch.Tensor) -> torch.Tensor:
        ...
        magnitude_curve = torch.exp(
            torch.cumsum(torch.randn(T, device=x.device) * self.magnitude_warp_sigma, dim=0)
        )
    ...
    def scaling(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        B = x.shape[0]
        scale_factor = torch.rand(B, device=x.device) * (self.scaling_range[1] - self.scaling_range[0]) + self.scaling_range[0]
    ...
    def temporal_masking(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for b in range(B):
            ...
            if n_mask > 0:
                mask_start = torch.randint(0, T - n_mask + 1, (1,), device=x.device).item()
                ...
                if mask_start > 0 and mask_start + n_mask < T:
                    for d in range(D):
                        ...
                        interp_vals = torch.linspace(start_val, end_val, n_mask + 2, device=x.device)[1:-1]
    ...
    def velocity_perturbation(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if D >= 2:  # dx, dy available
            ...
            noise_dy = (correlation * noise_dx + 
                       np.sqrt(1 - correlation**2) * torch.randn(B, T, device=x.device)) * self.noise_std
    ...
    def direction_jitter(self, x: torch.Tensor) -> torch.Tensor:
        ...
        jittered_x = x.clone()
        direction_noise = torch.randn_like(x[:, :, 2], device=x.device) * 0.05  # Small angular noise
        jittered_x[:, :, 2] += direction_noise
        
        # Keep direction in valid range [-π, π]
        jittered_x[:, :, 2] = torch.remainder(jittered_x[:, :, 2] + np.pi, 2 * np.pi) - np.pi
        
        return jittered_x
    
    def apply_augmentation(self, x: torch.Tensor, augmentation_type: str = 'random') -> torch.Tensor:
        """
        ...
        if augmentation_type == 'random':
            ...
            augmentations = [
                'noise', 'time_warp', 'magnitude_warp', 'rotation',
                'scaling', 'temporal_mask', 'velocity_pert', 'direction_jitter'
            ]
    ...
    def compose_augmentations(self, x: torch.Tensor, 
                            augmentations: List[str], 
                            probabilities: Optional[List[float]] = None) -> torch.Tensor:
        """
        ...
        for aug_type, prob in zip(augmentations, probabilities):
...
class AugmentationStrategies:
    ...
    def measurement_noise():
        ...
        return lambda x: aug.compose_augmentations(
            x, 
            ['noise', 'velocity_pert', 'direction_jitter'],
            [0.8, 0.6, 0.4]
        )
    ...
    def spatial_variations():
        ...
        aug = TrajectoryAugmentations(
            rotation_angle_range=0.2,
            scaling_range=(0.85, 1.15)
        )
        return lambda x: aug.compose_augmentations(
            x,
            ['rotation', 'scaling', 'magnitude_warp'],
            [0.6, 0.4, 0.3]
        )
    ...
    def comprehensive():
        ...
        return lambda x: aug.compose_augmentations(
            x,
            ['noise', 'rotation', 'scaling', 'velocity_pert'],
            [0.7, 0.4, 0.3, 0.5]
        )
    ...
    def conservative():
        ...
        aug = TrajectoryAugmentations(
            noise_std=0.008,
            rotation_angle_range=0.05,
            scaling_range=(0.95, 1.05)
        )
        return lambda x: aug.compose_augmentations(
            x,
            ['noise', 'direction_jitter'],
            [0.6, 0.3]
        )
