...
#     text_size=10, 
#     figsizemultiplier=5,  # Overall figure size, default adaptable to subplot size
#     time_window=config.TIME_WINDOW, 
#     overlap=config.OVERLAP
# ):
#     # Determine unique motion classes and assign colors using the specified colormap
#     motion_classes = time_windowed_df['motion_class'].unique()
    
#     # Use provided order or default to the natural order
#     if order:
#         motion_classes = [cls for cls in order if cls in motion_classes]
...
#     # Map each motion class to a specific color
...
#     fig, axes = plt.subplots(len(motion_classes), num_tracks, figsize=(figsizemultiplier * num_tracks, figsizemultiplier * len(motion_classes)), facecolor=figure_background)
...
#         i = list(motion_classes).index(motion_class)
#         j = idx % num_tracks
...
#         # Add motion class label only to the first plot in each row
#         if j == 0:
...
def plot_tracks_by_motion_class(
    ...
    # Calculate an optimal layout for each motion class
    fig_rows = len(motion_classes)
    fig_cols = num_tracks if num_tracks <= 10 else math.ceil(num_tracks / 2)
    subplot_rows = 2 if num_tracks > 10 else 1
    ...
    fig, axes = plt.subplots(fig_rows * subplot_rows, fig_cols, figsize=(figsizemultiplier * fig_cols, figsizemultiplier * fig_rows), facecolor=figure_background)
    fig.subplots_adjust(wspace=0.0001, hspace=0.0001)  # Tighter spacing
    ...
    for idx, (segment, info) in enumerate(zip(track_segments, track_info)):
        ...
        # Calculate subplot row and column index for multi-row display
        i = list(motion_classes).index(motion_class) * subplot_rows + (idx // fig_cols) % subplot_rows
        j = idx % fig_cols
